...
const Navbar = () => {
  ...
  return (
    ...
    >
        ...
        <nav className="hidden md:flex items-center gap-1">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="relative px-3 py-2 group text-foreground/80 hover:text-foreground transition-colors"
            >
              {link.name}
      ...
      <div
        className={cn(
          "fixed inset-0 bg-background z-40 transform transition-transform duration-300 md:hidden pt-20",
        ...
        <nav className="flex flex-col items-center justify-center h-full gap-4">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="text-xl font-medium px-4 py-3 hover:text-primary transition-colors"
